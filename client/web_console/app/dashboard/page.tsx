...
import {
  admin, openAdminSSE,
  type Overview, type Kiosk, type SseEvent, type TelemetryPort,
} from "@/lib/api";
      ...
      (event: SseEvent) => {
        ...
        if (event.type === "overview") {
          setOverview(prev => prev ? {
            ...
            active_charging:      event.active_charging       ?? prev.active_charging,
            total_credits_earned: event.total_credits_earned  ?? prev.total_credits_earned,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <h2 className="text-gray-800 font-semibold">
            ...
            {live && <span className="ml-2 text-xs text-green-600 font-normal">● streaming</span>}
          </h2>
          {kiosks.length === 0 && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 text-gray-400 text-sm">No kiosks found.</div>
          )}
          {kiosks.map(k => {
            ...
            return (
              <div key={k.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                {tel ? (
                      ...
                      {tel.portData.map(p => (
                        <div key={p.port} className="bg-gray-50 rounded-lg p-3">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-xs font-medium text-gray-500">Port {p.port}</span>
                            <span className={`w-2 h-2 rounded-full ${p.relay_on ? "bg-green-500" : "bg-gray-300"}`} />
                          </div>
                          <p className="text-sm font-bold text-gray-800">{(p.voltage_v * p.current_a).toFixed(1)}W</p>
                          <p className="text-xs text-gray-500">{p.current_a.toFixed(2)}A · {p.voltage_v.toFixed(1)}V</p>
                        </div>
                      ))}
                    </div>
                    <div>
                  ...
                  <p className="text-gray-400 text-sm">Waiting for telemetry...</p>
                )}
              </div>
            );
          })}
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <p className="text-gray-500 text-sm">Total users: <span className="font-semibold text-gray-800">{overview?.total_users ?? "—"}</span></p>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
