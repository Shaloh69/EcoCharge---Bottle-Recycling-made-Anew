...
export default function SettingsPage() {
  ...
  const handleSave = async () => {
    ...
    setSaved(false);
    try {
      const updated = await admin.saveSettings(settings as Record<string, string>);
    ...
    <div className="p-8">
      ...
      <div className="space-y-6 max-w-2xl">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <h2 className="text-gray-800 font-semibold mb-1">Credit Tiers (by volume)</h2>
          <p className="text-gray-400 text-xs mb-4">Credits awarded when a bottle is deposited, based on volume.</p>
          <div className="space-y-4">
            <div className="flex items-center justify-between py-3 border-b border-gray-50">
              <div>
                <p className="text-gray-700 text-sm font-medium">Small bottle (≤{settings.credit_tier_s_max_ml}ml)</p>
            ...
            <div className="flex items-center justify-between py-3 border-b border-gray-50">
              <div>
                <p className="text-gray-700 text-sm font-medium">Medium bottle ({settings.credit_tier_s_max_ml}–{settings.credit_tier_m_max_ml}ml)</p>
                <p className="text-gray-400 text-xs">Credits per deposit</p>
              </div>
              <div className="flex items-center gap-2">
                <input
            ...
            <div className="flex items-center justify-between py-3">
              <div>
                <p className="text-gray-700 text-sm font-medium">Large bottle (&gt;{settings.credit_tier_m_max_ml}ml)</p>
                <p className="text-gray-400 text-xs">Credits per deposit</p>
              </div>
              <div className="flex items-center gap-2">
                <input
        ...
        {/* Volume Thresholds */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <h2 className="text-gray-800 font-semibold mb-1">Volume Thresholds (ml)</h2>
          <p className="text-gray-400 text-xs mb-4">Define where S/M/L boundaries fall.</p>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-gray-500 block mb-1">S/M boundary (max ml for Small)</label>
              <input
            ...
            <div>
              <label className="text-xs text-gray-500 block mb-1">M/L boundary (max ml for Medium)</label>
              <input
        ...
        {/* Charging */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <h2 className="text-gray-800 font-semibold mb-1">Charging Settings</h2>
          <p className="text-gray-400 text-xs mb-4">Controls how long each credit lasts and energy budget per credit.</p>
          <div className="space-y-4">
            <div className="flex items-center justify-between py-3 border-b border-gray-50">
              <div>
                <p className="text-gray-700 text-sm font-medium">Energy budget per credit (Wh)</p>
                <p className="text-gray-400 text-xs">Actual Wh allocated; dynamic duration based on real draw</p>
              </div>
              <div className="flex items-center gap-2">
                <input
                  type="number" min="1" step="0.5"
                  value={settings.energy_budget_wh_per_credit}
                  onChange={e => set("energy_budget_wh_per_credit", e.target.value)}
            ...
            <div className="flex items-center justify-between py-3 border-b border-gray-50">
              <div>
                <p className="text-gray-700 text-sm font-medium">Base minutes per credit</p>
                <p className="text-gray-400 text-xs">Fallback when no telemetry available</p>
              </div>
              <div className="flex items-center gap-2">
                <input
                  type="number" min="1" step="1"
                  value={settings.base_minutes_per_credit}
                  onChange={e => set("base_minutes_per_credit", e.target.value)}
                  className="w-20 border border-gray-200 rounded-lg px-3 py-1.5 text-sm text-center focus:outline-none focus:ring-2 focus:ring-green-500"
                />
            ...
            <div className="flex items-center justify-between py-3">
              <div>
                <p className="text-gray-700 text-sm font-medium">Max charging duration</p>
                <p className="text-gray-400 text-xs">Hard cap regardless of credits</p>
              </div>
              <div className="flex items-center gap-2">
                <input
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <p className="text-gray-400 text-xs mb-4">Used to calculate gain/loss in analytics.</p>
          <div className="flex items-center justify-between py-3">
            <div>
              <p className="text-gray-700 text-sm font-medium">Electricity rate (PHP/kWh)</p>
              <p className="text-gray-400 text-xs">Meralco / local utility rate</p>
            </div>
            <div className="flex items-center gap-2">
              <input
                type="number" min="1" step="0.1"
                value={settings.electricity_rate_php_per_kwh}
                onChange={e => set("electricity_rate_php_per_kwh", e.target.value)}
                className="w-24 border border-gray-200 rounded-lg px-3 py-1.5 text-sm text-center focus:outline-none focus:ring-2 focus:ring-green-500"
              />
