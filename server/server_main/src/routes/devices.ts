...
router.get(
  "/commands",
  async (req: DeviceRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (commands.length > 0) {
        log.device(
          `kiosk #${kioskId} poll — ${commands.length} pending command(s): ${commands.map((c) => c.commandType).join(", ")}`,
        );
      }
    ...
    try {
      ...
      for (const session of activeSessions) {
        ...
        if (elapsed >= session.durationSeconds) {
          log.charging(
            `kiosk #${kioskId} — session #${session.id} expired (port ${session.portNumber}), auto-completing`,
          );
          await prisma.chargingSession.update({
            where: { id: session.id },
            data: { status: "completed", endedAt: new Date() },
          });
          await queueCommand(kioskId, "deactivate_port", {
            port: session.portNumber,
          });
          log.charging(
            `kiosk #${kioskId} — queued deactivate_port for port ${session.portNumber}`,
          );
        }
      }
      ...
      if (bottle_in_bin === true) {
        log.device(
          `[Stage 5] bottle_in_bin=true received from ESP32 — kiosk #${kioskId}, looking for pending deposit`,
        );
        ...
        if (pendingDeposit) {
          log.device(
            `[Stage 5] Confirming deposit #${pendingDeposit.id} — ${pendingDeposit.creditsAwarded} credits → user #${pendingDeposit.session.userId}`,
          );
          ...
          if (pendingDeposit.creditsAwarded > 0) {
            ...
            log.device(
              `kiosk #${kioskId} — awarded ${pendingDeposit.creditsAwarded} credits to user #${pendingDeposit.session.userId}`,
            );
          }
          ...
          log.deviceWarn(
            `kiosk #${kioskId} — bottle_in_bin=true but no pending deposit found`,
          );
        }
      } else if (bottle_in_bin === false && fsm_state === "confirming") {
        log.device(
          `kiosk #${kioskId} — bin timeout (fsm=confirming), rejecting pending deposit`,
        );
        ...
        if (pendingDeposit) {
          ...
          log.deviceWarn(
            `kiosk #${kioskId} — deposit #${pendingDeposit.id} rejected (bin timeout)`,
          );
        }
