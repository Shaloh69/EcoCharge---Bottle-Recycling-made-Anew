...
const _qrPending = new Map<
  string,
  { userId: number; sessionId: number; user: object; expiresAt: number }
>();
...
async function getPortStatus(kioskId: number) {
  ...
  if (latest?.portData) {
    try {
      ports = JSON.parse(latest.portData);
    } catch {
      ports = [];
    }
  }
...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      log.kiosk(
        `Session CREATE — user #${req.userId} → kiosk #${body.kiosk_id}`,
      );
      ...
      log.kiosk(
        `Session #${session.id} created — user #${req.userId} kiosk #${body.kiosk_id}`,
      );
...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      log.kiosk(
        `Deposit (legacy) — user #${req.userId} session #${body.session_id} brand=${body.brand ?? "?"} vol=${body.volume_ml ?? "?"}mL`,
      );
      ...
      if (!session) {
        log.kioskWarn(
          `Deposit failed — session #${body.session_id} not found for user #${req.userId}`,
        );
        res.status(404).json({ error: "session not found" });
        return;
      }
      ...
      const creditsAwarded = body.volume_ml
        ...
        : 0;
      log.kiosk(
        `Deposit — creditsAwarded=${creditsAwarded} for ${body.volume_ml ?? "?"}mL`,
      );
      ...
      await queueCommand(session.kioskId, "open_conveyor", {
        deposit_id: deposit.id,
      });
      log.kiosk(
        `Deposit #${deposit.id} confirmed — open_conveyor queued for kiosk #${session.kioskId}, credits=${creditsAwarded}`,
      );
...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!session) {
        log.kioskWarn(
          `Bottle approve failed — session #${body.session_id} not found for user #${req.userId}`,
        );
        res.status(404).json({ error: "session not found" });
        return;
      }
      ...
      const creditsAwarded = body.volume_ml
        ...
        : 0;
      log.kiosk(
        `Bottle APPROVE — creditsPending=${creditsAwarded}, queuing approve_bottle for kiosk #${session.kioskId}`,
      );
      ...
      await queueCommand(session.kioskId, "approve_bottle", {
        deposit_id: deposit.id,
      });
      log.kiosk(
        `[Stage 4] Deposit #${deposit.id} created (pending_bin) — approve_bottle queued for kiosk #${session.kioskId} | credits_pending=${creditsAwarded}`,
      );
...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      const body = z
        .object({ session_id: z.number().int().positive() })
        .parse(req.body);
      log.kiosk(
        `Bottle REJECT — user #${req.userId} session #${body.session_id}`,
      );
      ...
      if (!session) {
        log.kioskWarn(
          `Bottle reject failed — session #${body.session_id} not found for user #${req.userId}`,
        );
        res.status(404).json({ error: "session not found" });
        return;
      }
...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      log.kiosk(
        `QR-LINK — session #${session.id} created, token stored (expires 5m)`,
      );
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      log.kiosk(
        `QR-STATUS — linked! user #${user.id} (${user.email}) session #${entry.sessionId}`,
      );
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      req.on("close", () =>
        log.kiosk(`SSE client disconnected — kiosk #${kioskId}`),
      );

      const portStatus = await getPortStatus(kioskId);
      res.write(
        `data: ${JSON.stringify({ type: "ports", ports: portStatus })}\n\n`,
      );
    } catch (err) {
      next(err);
    }
