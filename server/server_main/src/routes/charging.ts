...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (activeOnPort) {
        log.chargingWarn(
          `START failed — kiosk #${body.kiosk_id} port #${body.port_number} already in use`,
        );
      ...
      if (user.creditBalance < body.credits) {
        log.chargingWarn(
          `START failed — user #${req.userId} insufficient credits (has ${user.creditBalance}, needs ${body.credits})`,
        );
      ...
      log.charging(
        `START — duration=${durationSeconds}s watt=${wattSnapshot ?? 0}W`,
      );
      ...
      log.charging(
        `Session #${chargingSession.id} started — activate_port queued for kiosk #${body.kiosk_id} port #${body.port_number}`,
      );
...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!session) {
        log.chargingWarn(
          `STOP failed — session #${id} not found or not active for user #${req.userId}`,
        );
      ...
      log.charging(
        `Session #${id} interrupted — deactivate_port queued for kiosk #${session.kioskId} port #${session.portNumber}`,
      );
