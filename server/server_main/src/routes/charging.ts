...
router.post('/start', requireAuth, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    log.charging(`START — user #${req.userId} → kiosk #${body.kiosk_id} port #${body.port_number} credits=${body.credits}`)
    ...
    if (activeOnPort) {
      log.chargingWarn(`START failed — kiosk #${body.kiosk_id} port #${body.port_number} already in use`)
      res.status(409).json({ error: 'port already in use' }); return
    }

    const user = await prisma.user.findUniqueOrThrow({ where: { id: req.userId! } })
    if (user.creditBalance < body.credits) {
      log.chargingWarn(`START failed — user #${req.userId} insufficient credits (has ${user.creditBalance}, needs ${body.credits})`)
    ...
    log.charging(`START — duration=${durationSeconds}s watt=${wattSnapshot ?? 0}W`)
    ...
    log.charging(`Session #${chargingSession.id} started — activate_port queued for kiosk #${body.kiosk_id} port #${body.port_number}`)
...
router.post('/stop/:id', requireAuth, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    log.charging(`STOP — user #${req.userId} session #${id}`)

    const session = await prisma.chargingSession.findFirst({
      where: { id, userId: req.userId!, status: 'active' },
    })
    if (!session) {
      log.chargingWarn(`STOP failed — session #${id} not found or not active for user #${req.userId}`)
    ...
    log.charging(`Session #${id} interrupted — deactivate_port queued for kiosk #${session.kioskId} port #${session.portNumber}`)
