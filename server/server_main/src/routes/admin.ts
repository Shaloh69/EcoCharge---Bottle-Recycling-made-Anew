...
// GET /kiosks
router.get('/kiosks', async (_req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/kiosks/:id/telemetry/latest', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!telemetry) { res.status(404).json({ error: 'no telemetry found' }); return }
    res.json({
      id: telemetry.id,
      kiosk_id: telemetry.kioskId,
      port_data: telemetry.portData ? (() => { try { return JSON.parse(telemetry.portData!) } catch { return null } })() : null,
...
router.get('/sessions', async (_req: Request, res: Response, next: NextFunction) => {
  try {
    const sessions = await prisma.kioskSession.findMany({
      ...
      take: 100,
      include: { user: { select: { name: true, email: true } }, kiosk: { select: { name: true } } },
    })
...
router.get('/charging', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const page = Math.max(1, parseInt((req.query.page as string) ?? '1'))
    ...
    const where = statusFilter
      ? { status: statusFilter as 'active' | 'completed' | 'interrupted' | 'error' }
      : {}
...
router.get('/alerts', async (_req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const alerts: Array<{ type: string; kiosk_id: number; kiosk_name: string; message: string; severity: string }> = []
    ...
    for (const kiosk of kiosks) {
      ...
      if (latest?.binLevel !== null && latest?.binLevel !== undefined && latest.binLevel >= 80) {
        alerts.push({
          type: 'bin_full',
          kiosk_id: kiosk.id,
          kiosk_name: kiosk.name,
...
router.get('/ml-review', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const page = Math.max(1, parseInt((req.query.page as string) ?? '1'))
    ...
    const [deposits, total] = await Promise.all([
      ...
      prisma.bottleDeposit.count({ where: { confidence: { lt: threshold } } }),
    ])
  ...
  try {
    ...
    const [chargingSessions, earnTransactions] = await Promise.all([
      prisma.chargingSession.findMany({
        ...
        select: { startedAt: true, wattSnapshot: true, durationSeconds: true },
      }),
      prisma.creditTransaction.findMany({
    ...
    // Build date map
    const dateMap: Record<string, { kwh_consumed: number; credits_issued: number }> = {}
    ...
    for (const session of chargingSessions) {
      ...
      if (!dateMap[dateStr]) dateMap[dateStr] = { kwh_consumed: 0, credits_issued: 0 }
      if (session.wattSnapshot && session.durationSeconds) {
        dateMap[dateStr].kwh_consumed += (session.wattSnapshot * session.durationSeconds) / 3600 / 1000
      }
    }
    ...
    const analytics = Object.entries(dateMap)
      ...
      .map(([date, data]) => {
        ...
        const gain_php = (data.credits_issued * energyBudgetWhPerCredit / 1000) * electricityRate - cost_php
        return {
          date,
