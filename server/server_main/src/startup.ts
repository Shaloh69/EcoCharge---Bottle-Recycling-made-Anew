...
export async function pingAIServer() {
  ...
  // ── Step 1: reachability (/health — no auth needed) ───────────────────────
  ...
  try {
    ...
    if (res.ok) {
      ...
      log.aiWarn(`Step 1 ✖ /health returned ${res.status} — server may be starting up`)
    }
  } catch (err) {
  ...
  // ── Step 2: auth check (/api/detect with empty body — expects 400 not 401) ─
  ...
  try {
    ...
    if (res.status === 401) {
      ...
      log.aiError(`  Node server sends : Bearer ${apiKey.slice(0, 8)}...${apiKey.slice(-4)}`)
      log.aiError(`  AI server replied : ${detail}`)
      log.aiError(`  → Check AI_API_KEY on both Render services matches the AI server .env`)
    } else if (res.status === 400 || res.status === 422) {
      // 400/422 = auth passed, just rejected bad/empty input — key is correct
      log.ai(`Step 2 ✔ AUTH OK (${res.status} = auth passed, empty body rejected as expected)`)
      log.ai('AI server fully operational ✔')
    } else if (res.ok) {
      log.ai(`Step 2 ✔ AUTH OK — server returned ${res.status}`)
    } else {
      log.aiWarn(`Step 2 ? unexpected status ${res.status} — check AI server logs`)
    }
  } catch (err) {
