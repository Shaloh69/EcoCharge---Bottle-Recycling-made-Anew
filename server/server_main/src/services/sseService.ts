...
export function broadcastToKiosk(kioskId: string, data: object): void {
  ...
  log.kiosk(`[Stage 6] SSE broadcast → kiosk #${kioskId} | type=${type} clients=${count} | ${JSON.stringify(data).slice(0, 120)}`)

  if (count === 0) {
    log.kioskWarn(`[Stage 6] No SSE clients connected for kiosk #${kioskId} — kiosk web may miss this event`)
  }
