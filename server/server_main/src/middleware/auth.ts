...
export function requireAuth(req: AuthRequest, res: Response, next: NextFunction): void {
  ...
  if (!token) {
    console.warn(`[Auth] 401 NO TOKEN — ${req.method} ${req.path} origin=${req.headers.origin ?? '?'}`)
  ...
  try {
    ...
    console.warn(`[Auth] 401 INVALID TOKEN — ${req.method} ${req.path} reason="${reason}" token_prefix="${token.slice(0, 12)}..."`)
