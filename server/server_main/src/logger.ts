...
export const log = {
  // ── Infrastructure ──────────────────────────────────────────────────────────
  startup(msg: string) {
    console.log(`${ts()} ${tag('Startup', C.bGreen)} ${C.bGreen}${msg}${C.reset}`)
  },
  ...
  aiWarn(msg: string) {
    console.warn(`${ts()} ${tag('AI', C.bYellow)} ${C.yellow}⚠  ${msg}${C.reset}`)
  },
  aiError(msg: string) {
    console.error(`${ts()} ${tag('AI', C.bYellow)} ${C.bRed}✖  ${msg}${C.reset}`)
  },
  ...
  deviceWarn(msg: string) {
    console.warn(`${ts()} ${tag('Device', C.bCyan)} ${C.yellow}${msg}${C.reset}`)
  },
  ...
  chargingWarn(msg: string) {
    console.warn(`${ts()} ${tag('Charging', C.magenta)} ${C.yellow}${msg}${C.reset}`)
  },
  ...
  warn(domain: string, msg: string) {
    console.warn(`${ts()} ${tag(domain, C.yellow)} ${C.yellow}⚠  ${msg}${C.reset}`)
  },
  error(domain: string, msg: string) {
    console.error(`${ts()} ${tag(domain, C.bRed)} ${C.bRed}✖  ${msg}${C.reset}`)
  },
  ...
  request(method: string, path: string, status: number, ms: number) {
    ...
    const statusColor  = status < 300 ? C.bGreen : status < 400 ? C.cyan : status < 500 ? C.yellow : C.bRed
