...
**Reconfirmed explicitly by the user, 2026-08-11: "REMEMBER I AM OKAY WITH FREE DOMAIN" — use the free Cloudflare Tunnel + free-domain path for everything public-facing, including the Node API, not just `kiosk_web`.** Stated reasoning: "we are not the only users to use the system" — this is heading toward real multi-user, multi-kiosk operation, not a single-developer demo, so the API needs the same stable, non-rotating public path as the kiosk. This doesn't change the technical design (the API was already scoped as Cloudflare-Tunnel-public in the table below, since the ESP32/mobile app both need it) — it's a deliberate reaffirmation, worth recording so a future session doesn't second-guess "is a free domain actually acceptable for something meant to have real users" and hold back on it.

**Resolved 2026-08-11, differently than this section originally specified.** Registering `dpdns.org` and creating a named tunnel both require interactive access to external accounts/dashboards Claude Code can't drive on its own — this was surfaced to the user plainly, along with the real fact that **Cloudflare itself does not offer any free *stable* hostname**: the only fully-Cloudflare-native free option is the quick tunnel (`cloudflared tunnel --url ...`, no account needed), and it's ephemeral — a new random `*.trycloudflare.com` subdomain on every restart. Given that choice, **the user picked the quick tunnel over the named-tunnel-plus-owned-domain path.** Live now: `cloudflared tunnel --url http://localhost:30010` on `desktop-gklhcri`, persistent via the same Task-Scheduler pattern as the Node API itself (task `EcoChargeTunnelAPI`). Verified reachable from the public internet, not just the tailnet.

**Real consequence of this choice, not hidden:** the URL rotates on every tunnel restart, which is fundamentally incompatible with the ESP32's compile-time `RENDER_BASE_URL` and a shipped Flutter app default — both need something that doesn't change out from under already-deployed hardware/apps. **Deliberately not done as a result:** no firmware reflash with this URL, no Flutter default update. This works fine for `kiosk_web` (once it's actually deployed to a field PC — not yet) and for verification/admin use. If real field deployment becomes imminent, this specific tradeoff is worth revisiting with the user — a named tunnel is the actual fix, and remains free modulo the one-time third-party subdomain signup this session avoided per their choice.
...
6. **Second real gap, found immediately after step 5, that actually took the live API down in a crash loop**: `src/index.ts` binds the HTTP port *before* running migrations, then `src/startup.ts`'s `runMigrations()` runs `prisma migrate deploy` on **every** process startup (not once) and calls `process.exit(1)` on any unhandled failure. A schema created via `db push` has no `_prisma_migrations` history, so `migrate deploy` throws `P3005: database schema is not empty` on the very first startup after the bootstrap — a real, expected state given how the DB was created, not a real error. Because the port was already bound, `/health` briefly answered before each restart, then the process died and the Task-Scheduler `.bat` loop respawned it every 5s, forever — genuinely live but genuinely broken, easy to miss with a single spot-check. **Fixed properly, not just patched around**: `runMigrations()` already had matching self-healing logic for two sibling error codes (`P3009` — rolls back and re-marks a failed migration; `P3018` — marks one migration applied on schema drift) — added the same pattern for `P3005`, which baselines *every* migration in `prisma/migrations/` as applied (reading the directory directly, since P3005 doesn't name a specific migration the way P3009/P3018 do) and retries. This means any *future* fresh-DB bootstrap via `db push` self-heals on first real startup instead of crash-looping — don't manually run `prisma migrate resolve --applied` for this again, the code does it now.
