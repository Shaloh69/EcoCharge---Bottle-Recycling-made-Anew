...
- [!] **§1.1 Cloudflare Tunnel + free domain — reconfirmed explicitly by the user 2026-08-11 ("REMEMBER I AM OKAY WITH FREE DOMAIN"), scope now explicitly includes the Node API, not just kiosk web ("we are not the only users to use the system").** This is a green light on the *approach*, not something Claude Code can execute unassisted: registering a `dpdns.org` subdomain and creating a named Cloudflare Tunnel both require interactive access to external accounts/dashboards that don't exist yet and can't be signed up for autonomously. **Need from you**: either do the `03-revamp-master.md` §1.1 setup-sequence steps 1–3 yourself (register the subdomain, add it to Cloudflare as a zone, create the tunnel) and hand back the dashboard-issued token, or say if you'd rather this be walked through interactively. Once a token exists, routing the Public Hostnames and running `cloudflared service install <token>` on `desktop-gklhcri` is something this session can do.
- [ ] §1.1 step 5: confirm the origin isn't reachable outside the tunnel — blocked on the tunnel existing
