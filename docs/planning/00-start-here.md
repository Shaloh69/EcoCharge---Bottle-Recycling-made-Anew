...
There's also `docs/planning/07-ai-detection-improvements.md` — a real, code-grounded diagnosis of a reported problem (bottles not detected properly while inside the conveyor), plus a dataset-expansion plan with real, verified Roboflow/Kaggle/TACO sources. It's independent of the migration work above and can proceed in parallel; its one firmware-touching item (a "nudge complete" signal) should be reviewed alongside the two already-paused firmware fixes, not flashed separately.

**`docs/planning/08-master-checklist.md` is the current, real source of truth for exact status — consult it before assuming anything below is done.** It consolidates every actionable item across this whole `docs/planning/` folder with honest, re-checked status (built 2026-08-11 after a self-audit found `DESIGN.md` had items marked done without the screenshot verification `02-design-mandate.md` §0 requires — see `memory.md`). Work it top to bottom; it has its own stop-conditions marked `[!]`.
