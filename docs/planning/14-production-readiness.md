...
- [x] **Database backups — BUILT AND RESTORE-TESTED 2026-09-03.** Was the single largest non-safety risk; there were literally zero backups.
  ...
  - [ ] **Still not off-*machine*.** The mirror protects against a dead disk, not a dead machine, a fire, or ransomware. The intended target was the kiosk PC, but it is **offline — last seen 8 days ago** (verified 2026-09-03 via `tailscale status`), so it cannot receive anything. Revisit when it is back, or pick another always-on tailnet host.
