...
- [x] **Database backups — BUILT AND RESTORE-TESTED 2026-09-03.** Was the single largest non-safety risk; there were literally zero backups.
  ...
  - [ ] **Copies are still on the same machine as the database.** A backup on the same disk is not a backup — it survives a bad migration or a dropped table, not a disk failure. Needs an off-box copy (the kiosk PC is on the tailnet and is the obvious target).
...
- [!] **Docker Desktop does not start at boot — this is the actual cause of the multi-hour outages, and it needs a decision.** Verified 2026-09-03: the host rebooted at **11:48**, but Docker Desktop did not start until **13:53:51**, when someone logged in. MySQL therefore did not exist for **two hours and five minutes**, and the API had nothing to connect to. The container's own restart policy is correct (`unless-stopped`) — it simply cannot run without the engine.
  - Confirmed by inspection: Docker Desktop's `AutoStart` setting is **`False`**, it sits in the per-user `Run` key (so it starts **at interactive login, not at boot**), and Windows auto-login is **not** configured.
  - The crash-loop fix above makes this *quiet* rather than *solved* — the API now waits calmly instead of restarting 41,677 times, but after ~2.5 minutes it still gives up, and **the database still is not there until a human logs in**.
  - **The options, and the tradeoff, are the user's call:**
    1. **Enable Windows auto-login for `transfer`** — makes unattended reboots work, but stores a password as an LSA secret and leaves a logged-in desktop on the machine holding the database.
    2. **Move MySQL off Docker Desktop** — a native MySQL service, or Docker Engine as a Windows service, both genuinely start at boot. More work, changes the deployment.
    3. **Accept it** and treat every reboot as needing a manual login. Workable for a bench, not for a kiosk left running at a university.
