...
- [!] **A replacement ESP32-B board is now a prerequisite for everything below.** Confirmed faulty 2026-09-03 by one-at-a-time testing on the same PC and cable: ESP32-A wrote and verified a 1,065,840-byte image, while ESP32-B could not verify a 26,800-byte bootloader across four attempts at three different baud rates. Its flash does not retain what is written. **ESP32-A is flashed, verified and stable** — the flashing procedure itself is proven and the binaries are staged on the kiosk PC.
...
- [!] **Liveness checks that probe, not just "is the task Running" — exposed by the 2026-09-03 brownout.** The AI server hung during startup: its Scheduled Task reported `Running`, four `python` processes existed, and `restarts.log` had **zero** entries — yet port 30012 was never bound and the service was dead for six minutes. **The `.bat` restart loop only catches a process that EXITS; a process that hangs is invisible to it.** Every service here has the same blind spot. The fix is a watchdog that curls each `/health` and restarts on failure, not on exit.
