...
- **Admin Console → [Mantine](https://mantine.dev/)** — genuinely different theming architecture from HeroUI (prop-driven, not Tailwind-variant-based), 120+ components, strong dark-mode support out of the box, and real dense-table/chart components suited to "Operations Console." **Correction, 2026-08-11: this section previously justified the choice by pointing at "a comparable dense ops dashboard on a sibling project" (EngiRent) — too close to "because EngiRent did it," a real risk of the two theses' admin tools reading as the same product re-skinned.** The library choice itself stands on its own merits (above) and isn't being reversed.
  **Second correction, 2026-08-12: the "independent" structural references that 2026-08-11's fix pointed at weren't independent at all** — a direct line-by-line comparison against EngiRent's own `02-design-mandate.md` §3 found that both `design-sparx/mantine-analytics-dashboard` and `qqharry21/nextjs-mantine-dashboard` are EngiRent's own primary/alternate Admin Console references too, and TailAdmin (this document's line just below) is EngiRent's secondary reference as well — three-for-three overlap. Pointing at "the same upstream template EngiRent also uses" produces the identical convergence risk by a different path; it isn't actually a fix. **Real replacements, genre-matched to what this console actually does (watching live device/telemetry state, not generic CRUD) instead of generic "analytics dashboard" templates:**
  - **[Signal](https://github.com/bytefer/signal-dashboard)-style terminal-inspired ops-monitoring dashboards** (search "Signal dashboard Next.js shadcn" for current forks/demos — the pattern, not a specific pinned repo, is what matters here) — Next.js + shadcn/ui, domain-specific monitoring views (fleet/device status, incidents, deployments) built around live state rather than static reports. Use for information architecture only (it's shadcn-based, not Mantine — same "borrow layout, not code" treatment already used elsewhere in this document for non-Mantine references).
  - **[ThingsBoard](https://github.com/thingsboard/thingsboard)** (Apache 2.0, a real production IoT platform, not a UI template) — not a code source, but the right reference for what an actual IoT device-fleet console's information architecture looks like: device list → device detail → live telemetry widgets → alarm/alert views. This is a much closer domain match for "kiosks are IoT devices reporting live telemetry" than any generic SaaS admin dashboard, and neither EngiRent nor any generic-admin-template list touches this genre at all.
  Use these for layout/IA ideas, not EngiRent's own admin console and not the two Mantine templates it also cites — don't cross-check against EngiRent's admin UI for "what looks right" while building this one; the dark "Operations Console" identity, IBM Plex Mono telemetry, and eco-green/volt-amber status system (§3) already give this surface a distinct enough visual identity that it shouldn't need to.
...
**Deliverable: keep `docs/planning/08-master-checklist.md` Phase E current**, recording the design system as actually implemented (not aspirational), with `docs/design-screenshots/README.md` holding the before/after screenshots per surface and a short rationale useful for the thesis write-up. Do not check off an item in Phase E without the screenshots to back it up.
...
**Open item, not yet specified — flag before building, don't invent it silently:** unlike a spacing/radius scale being explicitly locked (e.g. an 8px grid with a hard radius cap), this document doesn't currently define one for EcoCharge. Given "untouched shadcn/HeroUI defaults" is banned and both apps already carry a customized `hero.ts` theme, check what radius/spacing that theme already commits to before inventing a new scale — but if it turns out to be unset or inconsistent, that's a real open question for the user, not something to guess at silently.

**Light and dark are each independently authored palettes, strictly enforced — added 2026-08-12, direct instruction, follow to the letter.** Every surface that ships both modes (Admin Console's light mode, Mobile App, Website) gets its own explicit light-mode AND dark-mode hex values below — never derive one by algorithmically inverting the other's lightness, and dark mode is never "the light theme with darker grays." This is the same discipline EngiRent's own mandate locked in its §1.3b, applied here with EcoCharge's own hue family (eco-green/volt-amber/signal-red/info-blue/bloom-violet), not copied values — and it closes a real gap: this document previously only specified the Admin Console's dark ramp, with light mode left as "exists" but undefined.

**Admin Console (dark is default; light mode is real, not a stub):**

| Token | Dark | Light |
|---|---|---|
| App background | `#0A0F0D` | `#F4FAF6` |
| Panel | `#111816` | `#FFFFFF` |
| Raised/soft surface | `#1A2420` | `#EAF7EF` |
| Border | `#243830` | `#D7E9DE` |
| Text primary | `#E7F0EB` | `#12261B` |
| Text secondary | `#8FA69B` | `#4C6358` |
| Text muted | `#5C7268` | `#7C9389` |

Deliberately close-but-distinct from the Kiosk's light values (`#F6FBF7`/`#FFFFFF`/`#14231B`, §4) — both read as the same green-tinted-white product family without literally sharing one token set; the Admin Console's dense typography, IBM Plex Mono telemetry, and status-color system (§3) keep the two surfaces from converging regardless.

**Mobile App + Website (share one light/dark ramp, per the existing "same palette/meaning" decision in §5/§6 — this is deliberate cross-surface consistency *within* EcoCharge, not the EngiRent-overlap problem flagged elsewhere in this document):**

| Token | Light | Dark |
|---|---|---|
| Background | `#F7FBF8` | `#08150F` |
| Surface | `#FFFFFF` | `#102A1E` |
| Soft surface | `#E9F5EC` | `#173A29` |
| Border | `#D2E8D8` | `#204F3A` |
| Ink (text primary) | `#122A1D` | `#EAF6EE` |
| Muted | `#587065` | `#7FA895` |

**Hard rule, mirroring EngiRent's §1.3b exactly — the same failure mode applies to any hue, not just blue: a light-mode primary/accent may never be used unlifted on a dark ground.** `eco-green-500 #16A34A` is tuned for light backgrounds; on the dark ramp above, dark-mode primary actions use a lifted variant, `#34D399`. Same treatment for `volt-amber-400 #FBBF24` if contrast against the dark ramp needs it (verify directly, don't assume it's fine) and for `bloom-violet` wherever it appears on a dark surface. Check every dark-mode primary/accent against its real background at build time.

**Kiosk stays deliberately light-only, unchanged from the existing decision** — a fixed public display in one known lighting environment has no per-user preference to persist, the same reasoning that already excluded it from needing a dark mode.

**Verification (extends §0): both modes must be screenshot-verified independently for every surface that ships them.** A screenshot taken only in light mode is not evidence dark mode works, and the reverse is equally true — this is the exact gap that shipped a real bug on EngiRent (light-mode cards left hardcoded white while the page background followed the OS to near-black); don't repeat it here by assuming symmetry instead of checking it.
...
Dense, monitoring-first, **dark-mode-first**. The console's actual job is watching live SSE telemetry (`docs/planning/09-system-analysis.md` §10's dashboard pages: overview, kiosks + kiosk detail, sessions, deposits, charging, credits, users, alerts, ml-review, analytics, settings), not marketing.
...
- **Status colors drive everything:** kiosk online/offline badges, bin gauge (green < 80%, amber 80–94%, red ≥ 95% — thresholds match the server's real alert logic in `docs/planning/09-system-analysis.md` §8's `/alerts` endpoint exactly, don't invent separate UI thresholds), port active/idle/error, ML-review confidence flags (amber < 0.70, matching the server's real default threshold), command PENDING (amber) / ACKED (green) / FAILED/EXPIRED (red).
...
**Templates to pull structure from, not code verbatim — corrected 2026-08-12, TailAdmin dropped because EngiRent's own mandate cites it too:** use the Signal-style terminal-monitoring pattern and ThingsBoard's real device-fleet IA (both named in the "Component library" correction above) — evaluate specifically for real-time data-display components (live SSE-driven tables/badges, device/alarm lists), since this console's job is watching live telemetry, not CRUD.
...
Build with `react-step-wizard`. Map `docs/planning/09-system-analysis.md` §6 directly into visible steps, not hidden state:
...
5. **Bin confirmation** — its own explicit shorter wait state, not folded silently into the result screen, since this can independently time out and flip to rejected (`docs/planning/09-system-analysis.md` §6, step 8).
...
- **[Self Service Kiosk](https://www.figma.com/community/file/1475972798185896799/self-service-kiosk)** (Figma Community) — general self-service touchscreen patterns. **Flagged 2026-08-12: this exact file is also cited in EngiRent's own `02-design-mandate.md` §4.6** — kept here only as a minor secondary reference for generic touch-target conventions, not layout authority; it is explicitly non-load-bearing, since §4.6 below already establishes the real, deck-derived layout language (pill shapes, wave/blob divider, bin-gauge, station grid) as this surface's actual authority, and that deck is EcoCharge-specific with no EngiRent equivalent.
...
**Read this in full before writing any kiosk UI code.** The deck is a Figma export at true kiosk resolution (1080×1920 portrait, confirmed from the PDF's own page geometry) showing the same set of roughly 15–18 unique screens repeated across several mock accounts and one language toggle — not 80 distinct screens. What follows is the real, observed design language, not a restatement of the generic tokens earlier in this document; **where this section and §2's shared token table disagree, this section wins for the Kiosk specifically** — update §2's shared token table to match, don't quietly average the two.
...
**Explicit template references, added 2026-08-12 — this section previously had none, unlike every other surface in this document, and unlike EngiRent's own Phone App section (§2), which names real templates explicitly. Cross-checked directly against EngiRent's Phone App references (Car Rental Mobile App UI Kit, Rental App UI Kit, Properties & Rental App Mini UI Kit, FlutterShop, flutter_ecommerce_template, E-commerce-App-UI-Flutter) — all e-commerce/rental-browsing genre. None of the below overlap; this app's real job is "check a balance, watch a live charge," a wallet/rewards problem, not a browse-and-buy one:**
- **[Loyalify — Loyalty App UI Kit](https://www.figma.com/community/file/1325324566787854013/loyalify-loyalty-app-ui-kit)** (Figma Community, free, 195+ screens) — primary reference, a gamified points/rewards pattern that's a close structural match for "deposit a bottle, watch credits accrue."
- **[Wallet App UI Kit](https://www.figma.com/community/file/1206901322111026232/wallet-app-ui-kit)** (Figma Community) — formalizes a direction already informally adopted in `docs/design-screenshots/README.md`'s researched-references table (hero balance number + transaction list, sourced from Mobbin's wallet-balance screen collection) — now locked here in the mandate itself, not just the evidence doc.
- **[Rewards and Discounts App UI Kit](https://www.figma.com/community/file/1289484749112823438/rewards-and-discounts-app-ui-kit)** (Figma Community) — supporting reference for the credits/rewards visual language specifically.

Use these for layout/component-pattern ideas, the same "adapt, don't clone" treatment already applied to every other template reference in this document — re-themed to eco-green/volt-amber/bloom-violet, not their default palettes.

- **Theme:** light + dark, eco-green primary, volt-amber accent reserved for charging/energy surfaces (charging screen, live wattage, port-state indicators). **Light/dark hex values locked in §2's new dual-palette table (2026-08-12) — use those, not an inverted guess.**
...
- **Animated background, added 2026-08-12 — this section previously specified none at all, the one gap left after the Kiosk/Website falling-leaves decision (§4.5) never got carried to Mobile.** **[`flutter_floating_particles`](https://pub.dev/packages/flutter_floating_particles)** (pub.dev, real published package) ships a pre-built `ParticleConfig.fallingLeaves` configuration — use it, recolored to eco-green/volt-amber per §4.5's existing rule (not literal autumn colors), on auth/onboarding screens (mirroring where EngiRent's Phone App puts its own `mesh_gradient` mesh-gradient background, §1.5 of that document — same *placement*, deliberately different *technique*: a particle system, not a fragment-shader gradient, so the two apps' auth screens don't converge even though both animate). This also completes the "falling leaves" motif across all three consumer-facing surfaces (Kiosk, Website, now Mobile) that already share the "Clean Energy Reward" identity — a real, deliberate cross-surface consistency win, distinct from the EngiRent-overlap problem this document corrects elsewhere. Same hard constraints as §4.5: `prefers-reduced-motion`-equivalent (Flutter's `MediaQuery.disableAnimations`) freezes it to a static scatter, never behind form fields at full strength, graceful degradation if the particle layer fails to init.
...
**Reference point for screen inventory:** `docs/planning/09-system-analysis.md` §13's current real screen list (splash/onboarding → login/register → home → scan kiosk QR → credit balance/transactions → deposit history → charging view/stop → profile) is the floor, not the ceiling — this redesign pass rebuilds the presentation layer of each of these, it doesn't need to invent new ones unless a genuine gap surfaces during the rebuild.
...
**Pages to build, using Taxonomy's real docs/blog structure and OpenStacked's page inventory as the starting skeleton (correction, 2026-08-12: this line previously still said "using Velora UI's real page set" — a leftover from before the 2026-08-11 correction above that explicitly moved away from Velora's page compositions; fixed to match):**
...
- **Download** — the mobile app. **No evidence this app is published to any app store** (checked: `docs/planning/09-system-analysis.md`/`docs/planning/10-paper-vs-repo-gap.md` never mention one) — build this as a direct APK download page (Velora UI's own `/docs`-adjacent download-flow pattern, or a simple dedicated route) rather than linking a store listing that doesn't exist. If a real store listing ever does get published, repoint this page then — don't build toward an assumed listing now.
