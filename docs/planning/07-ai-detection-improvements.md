...
**Decision (2026-08-10): merge this dataset and retrain. Two things resolved, one still blocking:**
- **Where to train — resolved.** Checked both machines' actual hardware: `desktop-gklhcri` has an AMD GPU only (no CUDA); the dev machine (`minniedumpor`) has the CUDA-capable RTX 3050 `SELF_HOSTING.md` was written against. Train on `minniedumpor`, not `desktop-gklhcri` — consistent with where the AI inference server already runs, per `analyzation.md` §15.
- **Merge mechanics — straightforward once downloaded.** Same single-class taxonomy (`plastic bottle`/`plastic-bottle`) as `Eco-Charge.v1`, so this is a class-compatible merge (no remapping needed, unlike the external Roboflow sets in §4.2) — combine images/labels into the existing train/val/test split proportionally, same discipline as §4.4's merge plan.
- **Still blocking: no Ultralytics HUB credentials found on either machine.** Can't pull the actual image files from the platform without an API key with access to this dataset — from the user directly, or from `jobert-vidad`. Nothing to merge until this is resolved.
